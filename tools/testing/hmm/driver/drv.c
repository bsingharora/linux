...
static void cdmdev_page_free(struct hmm_devmem *devmem, struct page *page)
{
	...
	cdmdev = dev_get_drvdata(devmem->device);
	pr_warn("freeing pfn %lx\n", page_to_pfn(page));

	mutex_lock(&cdmdev->lock);
	bitmap_clear(cdmdev->bitmap, page_to_pfn(page) - (unsigned long)devmem->pfn_first, 1);
	mutex_unlock(&cdmdev->lock);
}
...
static int cdm_device_probe(struct device_node *dn)
{
	...
	cdmdev->hmm_devmem = hmm_devmem_add_resource(&cdmdev_ops,
					&cdmdev->hmm_device->device,
					&cdmdev->res);
	if (!cdmdev->hmm_devmem)
		goto err;

	cdmdev->bitmap = kzalloc(BITS_TO_LONGS(cdmdev->hmm_devmem->pfn_last + 1 -
					cdmdev->hmm_devmem->pfn_first), GFP_KERNEL);
	if (!cdmdev->bitmap)
		goto err2;
	mutex_init(&cdmdev->lock);
